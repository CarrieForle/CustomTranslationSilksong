...
In most cases, you can get the text files of fan-made translation with export. Here's how:
...
3. Options > Mods > Custom Translation > Export current language
4. The game will open a folder containing the exported files. Copy `entry` folder to `translation`.
5. Under `translation`, rename `entry` folder to its language name.
6. Open `metadata.json` and replace `Language` field with the correct [language code](https://github.com/SFGrenade/LanguageSupport-Repo#table-of-available-language-codes).
7. Save and close `metadata.json`.
8. Return to Silksong and hit 'Reload translation'.
9. Go to language menu and see if it's there. You can tell by checking if there are two same languages. Or close Silksong, replace the game files with backup from step 1, and start Silksong to see if it's there.

In some case, you need to extract the files yourself because you can't start Silksong after replacing the files. This can be done with tools like [AssetRipper](https://github.com/AssetRipper/AssetRipper) and [AssetStudio](https://github.com/Perfare/AssetStudio/). You can follow the Indonesian example below for a walkthrough.
...
Before you downlod the font. Create a folder named `assets` under `UK`.
...
If it works. It's great! ...However you could not edit the translation text because they're actually **encrypted**. You need to decrypt it to edit those text.
...
A folder will be opened containing the exported files which are decrypted. Copy `entry` folder into `translation` folder in the mod installation folder. Remove `ID` folder and rename `entry` folder to `ID`. Now you can start the game and see if it works.
